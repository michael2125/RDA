...
for iDB = 1:length( Dataset_name )% DB is data base; and length(DB) is number of Data Base.
    ...
    load ('/home/yin/disk3/DATA_Fill/SSVEP_sub_044.mat');
    fprintf('Done\n');
    nClasses = length(unique(gnd));
    X = 1e5*fea;
    Y = gnd';
    % =====================================================================
    type_method = 'conj_SGRDA'; % as type = 'NTD_LE' & options.alpha = 0, that is NTD algorithm
    tensor_shape    = [24 401];% the dimension for dataset dependent reshaped tensors
    lowerdims = [8 100];
    % =====================================================================
    ...
    for rn = 1 : runtimes
        ...
        for cros = 1 : cross_num
            ...
            % =============================================================
            [P, ela] = RDA(train, train_label, lowerdims, tensor_shape);
            %[P, ela] = conj_RDA(train, train_label, lowerdims, tensor_shape);
